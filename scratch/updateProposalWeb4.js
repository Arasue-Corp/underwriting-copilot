...
if (startIndex !== -1 && endIndexStr !== -1) {
  const originalBlock = code.substring(startIndex, endIndexStr + '          })}\n'.length);
  
  const newBlock = `{groupedProposals.map((group: any, idx: number) => {
            const isSelected = selectedModules[group.options[0].originalIdx];
                    ...
                    <div className="mt-12 max-w-md print:hidden">
                      <button
                        ...
                        onClick={() => {
                          ...
                          next[group.options[0].originalIdx] = !next[group.options[0].originalIdx]
